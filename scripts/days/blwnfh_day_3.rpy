label blwnfh_day_3:
    ...
    dv "А вот при том, мне Ульяна рассказала, как вы мило гуляли[wp]"
    dv "Конечно, там были ещё некоторые детали, но в них я уже не поверила."
    ...
    me "А[wp] Зачем водка?"
    cs "Перекиси мало, водки много, так что в случае чего будете ей раны обрабатывать."
    ...
    me "Водку?"
    cs "Пакет."
    me "Ааа[wp] А что со столичной делать? Если не пригодится."
    ...
    sh "Только куда целый литр девать — непонятно."
    ...
    sh "Семён, ну какое воровство? {w}Там же не сложно подойти, и попросить бутербродов или булок да кефира."
    ...
    th "Но если Серый действительно где-то потерялся, то это мой единственный способ узнать где он сейчас."
    
    me "Что, детство в одном месте заиграло?"
    us "Ага!"
    me "Ладно, я согласен, но только если он не придёт в столовую."
    us "Так и не придёт!"
    ...
    th "Не зря она советовала присесть[wp]"
    ...
    th "Что, замучил тебя наш Серёжа? Так тебе вот и надо!"
    mz "Чего пришел?"
    ...
    "Я был удивлен ответу Жени, ведь не ожидал, что вот так вот она его сразу выдаст."
    "Спустя пару секунд из-за книжной полки вышел и сам наш герой."
    ...
    me "Да, кстати, Серый, тебя шеф вызывает."
    ...
    play music blwnfh_music_list["mystery"] fadein 5
    $ blwnfh_set_mode(nvl)
    nvl show dissolve 
    
    # надо бы это дело переделать... наверное....
    ...
    "Например, как великие нации динозавров воюют между собой за кусок хлеба, что был испечён в ядре планеты великим полубогом. {w}И хлеб этот, якобы давал некие супер силы. {w}Ну там, способность стрелять лазером из глаз, пулять огненные шары из рта. В общем всякое такое."
    "Полубог, кстати, по совместительству ещё был королём всей этой чудной планеты и наделил динозавров разумом[wp] {w}А ещё и оружием. Чисто забавы ради."
    "И так шли столетия, пока не прилетели инопланетяне из другой галактики. {w}Они убили полубога, подчинили динозавров и стали использовать их для работы своих двигателей, запрягая их, словно хомяков, в большие колёса, когда на другом коцне был кусок еды. {w}Между прочим, КПД у такой схемы было крайне высокое, куда лучше их прошлых двигателей на банановой кожуре.\n"
    "Но, в один момент всё закончилось."
    
    stop music fadeout 3
    
    "Буквально. {w}Я просто стоял по среди ничего, в кромешной тьме."
    
    nvl clear
    
    th "Что на этот раз? {w}Мозг решил придумать мне ещё и кошмар?"
    
    "\nНезаметно проскочила осознанная мысль."
    "Мне сразу стало понятно, что попал я в осознанное сноведение вот только[wp] {w}Как так?\n"
    
    th "Вроде как не практикавался, да и никто мне особо не помогал в этом деле[wp]"
    th "Хотя, имеет ли это сейчас хоть какое-то значение? Я же могу сейчас напридумывать себе всё что угодно!"
    th "Например, что я самый богатый человек мира или там[wp] {w}Самый умный!"
    th "Придумать чудесный мир, где все живут в мире! {w}А потом уничтожить его и всех обитателей ядерными бомбами!" 
    th "Или же стать Наполеоном! Всея Руси!"
    
    "\nРешая не терять такой шанс, я тут же попытался вообразить первую мысль что пришла на ум и[wp] {w}Ничего. Как бы я не старался."
    "Единственное, что добавилось к полной пустоте это необычное ощущение, будто я подвешен где-то высоко в воздухе на длинную верёвку. Только верёка неосязаемая."
    
    nvl clear
    
    th "Странное ощущение[wp] Похоже точно кошмар." 
    
    "\nЗдесь царила абсолютная тишина, не было каких-то ощущений, кроме чувства подвешенности. {w}НИ-ЧЕ-ГО. {w}И это пугало ещё сильнее[wp]\n"
    
    th "И чем же меня напугает мой первый в жизни осознанный сон? Какую больную идею выдаст моя фантазия?"
    th "Огнедышащая креветка была[wp] Было и[wp]"
    
    "\nНеожиданно, тишина была нарушена пронзительным, до мурашек, дверным скрипом."
    "Посмотрев в сторону, откуда донёсся звук, я увидел там проход. {w}Оттуда бил белый яркий свет, а ещё[wp] {w}Там стоял некий силуэт[wp]"
    "И судя по форме тела, да и вообще немалым размерам, это был мужчина."
    
    play music music_list["orchid"] fadein 5
    
    extend " Который стал медленно, угрожающе приближаться ко мне."
    "Что делать в такой ситуации, было не совсем понятно. Да и не мог я что-то сделать, но и просто наблюдать за фейрверками как-то тоже не очень хотелось."
    
    nvl clear
    
    "Я попытался хоть как-то пошевелиться, лишь бы выбраться из оков тьмы."
    "Однако, тень плотно меня держала и, видимо, не собиралась отпускать, так что вырваться никак не получалось."
    "А тем временем мужчина становился всё ближе и уже можно отчетливо расслышать его шаги, шум которых разнозился эхом."
    "Будучи вообще единственным здесь звуком, это нехило так давило на мозги и делало всю ситуацию только более ужасающей."
    "Но в один момент, раздался громкий грохот, будто что-то где-то взорвалось."
    "А затем ещё{w} И ещё. {w}Грохот был настолько громкий, что я чуть ли не оглох."
    "Но самое главное, что этот самый мужик остановился.\n" 
    
    th "Похоже, этот грохот немного оглушил его."
    
    "\nЗатем, раздался ещё, куда более громких грохот."
    
    stop music fadeout 3
    
    "Неизвестный стал быстро убегать обратно в свою дверь, где вскоре и скрылся, закрыв её со всё таким же мерзким скрипом."
    "И более за этим ничего не последовало.\n"
    
    th "Неужели это[wp] Закончилось?"

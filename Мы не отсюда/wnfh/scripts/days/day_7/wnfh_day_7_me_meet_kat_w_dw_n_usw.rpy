label d7_me_meet_kat_w_dw_n_usw:
    ...
    "Мы вышли на площадь, и Ульяна резко остановилась."
    ...
    show usw laugh pioneer at left with dspr
    ...
    th "Приехали уже[wp] Значит, времени на подготовку немного[wp] {w=1}Надеюсь, Ульяна быстро придёт."
    ...
    #КОСЯК: в этот момент спрайт Алисы, если верить превью, должен исчезнуть, но он остаётся на месте.
    ...
    dv "Ты с дуба рухнул? У нас времени мало, так что один раз."

    "Я вздохнул, мы занесли свои кулаки и под отсчёт «цу-е-фа» остановились."
    ...
    dv "Лезь давай. Заодно и обольёшь тогда."
    ...
    usw "Кончай вертеться и бери. Ты же согласился поучаствовать, а значит должен был быть готов ко всему!"
    ...
    "И как только из-за приоткрывшихся ворот показался силуэт, я сразу же выплеснул на него всё содержимое ведра."
    ...
    call screen wnfh_choice(
        ["kat", "Стоит извиниться", "Провинился — буду исправлять", "d7_sh_yes_1", {"kat":1}],
        ["neutral", "Меня подставили", "Пусть рыжие извиняются", "d7_sh_no_1", {"kat":-2}],
        ["d7_choice_n9", "Шурик требует от Семёна извниться перед Катей"]
...
label d7_sh_yes_1:
    ...
    kat "Вот как[wp]"
    me "Ну ладно, раз я тут в провинившихся перед тобой, пойдём хотя бы к складу провожу. Там тебе выдадут сухую форму."
...
label d7_sh_no_1:
    ...
    sv "Так-так, в наших рядах пополнение, как я понимаю."
    
    "Она указала рукой на новенькую."
    
    show sv angry pioneer glasses tablet at center with dspr

    sv "Только вот почему она вся мокрая?"
    ...
    "Света бросила свой тяжелый взгляд на меня."
    
    sv "Поскольку ты не смог сопроводить новоприбывшую в целостности и сохранности, сопровождение переходит под мою юрисдикцию, а тебя будет ждать наказание от вожатой, понял?"
    me "Понял."

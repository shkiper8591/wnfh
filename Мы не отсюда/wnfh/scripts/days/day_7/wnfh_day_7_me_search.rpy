label d7_me_search:
    ...
    th "Возможно, они у себя дома. Хотя в такую жару дома делать нечего."
    ...
    #Тут пока всё косо работает, на вторую по счёту локацию можно возвращаться сколько угодно, а если на пляж сначала пойти, можно ещё и трейс словить.
...
label d7_me_search_dv_sport:
    ...
    mi "Приветик, Семён! Ты тоже решил позаниматься спортом? Или просто понаблюдать за Славей? Знаешь, это так увлекательно! Она так много разных упражнений знает и так легко их выполняет!"
...
label d7_me_search_dv_house:
    ...
    #Если не подглядывать, музыка не замолкает, пока не начинается другая. Смотреть на Ленина и гулять с Леной под неё как-то странно.

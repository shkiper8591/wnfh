...
label d7_obed_me_w_un_kat:
    ...
    if wnfh_Data.FlagGet("me_neznayu_imya_kat") == True:
        ...
        "За своими раздумьями я не обращал внимания на увлечённо о чём-то болтающих Лену и новенькую."
        ...
        un "Вот как, ну что, знакомьтесь. Семён это Катя, Катя это Семён"

        $ wnfh_set_name("kat", "Катя")

        kat "Ага[wp]"

        "Катя без особого энтузиазма проговорила она."

        un "Вот и познакомились, а теперь, изволь Семён, мы продолжим наш диалог."
    ...
    "Спросил я невзначай, оторвавшись от порции супа."
    "Однако после моего вопроса они залились румянцем, будто я задал какой-то неприличный вопрос."

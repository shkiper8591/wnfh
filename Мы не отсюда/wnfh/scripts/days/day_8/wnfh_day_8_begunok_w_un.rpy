label d8_begunok_w_un:
    ...
    mz "Так, если хочешь брать здесь книжки, придётся заполнить читательский билет."
    ...
    th "С другой стороны, вожатая будет очень сильно ругаться на меня. Не исключаю, что матом."

    if wnfh_Data.FlagGet("d7_kat_oblivanie") == "me_oblil":

        th "Да и с Катей неудобно как-то выходит[wp] Я её облил, так и ещё бросаю вот. Нужно же хоть как-то отработать свой косяк."
...
label d8_begunok_w_un_flagcheck_dv_oblila:
    ...
    me "Хе, но она правда хороший друг. Только юмор у них с Ульяной своеобразный."
...
label d8_begunok_w_un_flagcheck_me_oblil:
    ...
    call screen wnfh_choice(
        ...
        ["Промолчать", "Я не канадец, чтобы помногу извиняться", "d8_begunok_w_un_silience", {"kat": -1}],
...
label d8_begunok_w_un_silience:
    ...
    "Пионерка не отрывала с меня с меня взор ещё несколько секунд. Но, поняв, что ничего она от меня не услышит, Катя также уткнула взгляд в землю."

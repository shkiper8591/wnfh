label d8_begunok_w_un:
    ...
    kat "Музыкальный кружок, клуб авиамоделирования, медпункт и библиотека."
    ...
    "Лена не сдержала смех, вгоняя нашу новенькую в полный ступор."
    ...
    mz "Мне казалось, только Семёна запрягли."
    ...
    "После своего несколько суховатого ответа Женя будто спохватилась, подняла взгляд на Катю и улыбнулась."
    ...
    "На этой приятной ноте библиотекарша вернула лист Кате."
    ...
    "Видимо, мысли унесли меня настолько далеко, что Лене пришлось возвращать меня в реальность криком, несколько напугав меня. И Катю в придачу."

    show kat normal pioneer at left with dspr
    show un normal pioneer at right

    me "А? Что?"
    un "Ты как-то многовато витаешь сегодня. Может, лучше оставишь обходной нам двоим?"
...
label d8_begunok_w_un_end1:
    ...
    me "Знаешь, да, пожалуй."

    "Помахав на прощание, я оставил девушек наедине, отправившись в клубы."
...
label d8_begunok_w_un_2:
    ...
    "Я усмехнулся со своей же шутки."
...
label d8_begunok_w_un_flagcheck_dv_oblila:
    ...
    if wnfh_Data.FlagGet("d7_me_pogulyal_w_un") == True:
        ...
        "Тем не менее, вчера мы хорошо прогулялись, пусть и недолго."
...
label d8_begunok_w_un_flagcheck_ne_oblil:
    ...
    me "Не знаю. Бревно бы уронили, например."

label d8_male_clubs:
    ...
    th "Если я хочу чем-то заняться, то мне к пацанам. У них всегда найдётся, что поделать."
    ...
    th "Скорее бы это уже всё закончилось[wp]"
    ...
    sh "Ладно, это всё потом, нам сейчас нужно оценить степень наших проблем."
    me "Это точно."
    ...
    "В целом, тут было ещё много разного, названий чего я просто мог не знать."
    ...
    "Я последовал за Шуриком, как Женя резко меня остановила и вручила ключ."
    ...
    if wnfh_Data.getChoice_result_number("d8_choice_n1") == 2:
        ...
        sh "Важно то, что он не функционирующий и его вряд ли могли разграбить какие-нибудь мародёры."

...
label d8_obed_me_kat_mi:
    ...
    mi "Конечно ладно, я же не говорю, что нужно прям обязательно идти в муз кружок, позабыв обо всём остальном."

    show kat smile with dspr

    kat "Спасибо тебе."
    mi "Ну так что, Семён, зайдёшь к нам?"
    me "Что ж, если дел не будет других, то, наверное, приду."

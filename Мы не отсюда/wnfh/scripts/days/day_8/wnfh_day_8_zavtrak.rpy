label d8_zavtrak:
    ...
    if wnfh_Data.getChoice_points_sum("un") <= 2:
        ...
        un "Семён, я заберу Катю? Надеюсь, ты не против, а то мне нужно кое-что с ней обсудить."
        ...
        th "Дилемма[wp] {w}С одной стороны братва, с другой — возможность наконец нормально поговорить с Мику."
...
label d8_zavtrak_s_miku:
    ...
    "Обычно после таких слов меня распирает любопытство, и я начинаю доставать людей просьбой рассказать, что же там такое."
    "Но за завтраком не стоит, да и Мику как-то доставать не хотелось."
    ...
    "Мику резко переменила тон на привычный задорный."
    ...
    call screen wnfh_choice(
        ...
        ["neutral", "Пожалуй, нет", "Не вижу особого смысла в этом", "d8_mi_no_1", {"mi":-1}],
...
label d8_zavtrak_s_el_sh:
    ...
    "Усевшись за стол, я осмотрел свой завтрак." 

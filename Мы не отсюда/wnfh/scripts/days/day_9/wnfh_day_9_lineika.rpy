label d9_lineika:
    ...
    if wnfh_Data.FlagGet("mt_angry") == True:
        ...
        th "Конечно, работа на складе получше обещанной вожатой покраски деревьев[wp] Но под руководством этой егозы? Увольте!"

        show usw laugh pioneer at cleft with dspr

        usw "Есть, гржнинначаник!"

        "Она радостно отсалютовала."
    ...
    "Как только пионеры заслышали про столовую, они мигом повернулись и отправились на завтрак."
...
label d9_lineika_w_usw:
    ...
    me "У меня к тебе тот же вопрос."
    ...
    if wnfh_Data.getChoice_result_number("d8_choice_n11") == 1:
        ...
        usw "А я думала, что за ночные песнопения."
        ...
        me "Видимо, об участии Алисы все решили забыть."
    ...
    me "Ты, кстати, сказала, что на складе по собственной инициативе."
    ...
    th "«Дедушке»? Она о Сергее Дмитриевиче? Не думал, что она настолько хорошо к нему относится."

label d9_zavtrak:
    ...
    elif wnfh_Data.getChoice_points_sum("mi")  >= 3:
...
label d9_zavtrak_w_kat_mi:
    ...
    "Зайдя в столовую, я, как обычно, взял себе поесть и стал искать, куда бы усесться."
    ...
    if wnfh_Data.FlagGet("mt_angry") == True:
        ...
        "Очевидно, Катя шутила, хоть сбежать действительно хотелось."
...
label d9_zavtrak_w_dv_usw:
    ...
    if wnfh_Data.FlagGet("mt_angry") == True:
        ...
        if wnfh_Data.getChoice_result_number("d8_choice_n11") == 1:
            ...
            "Тяжело вздохнув, я вернулся к завтраку."
            ...
            if wnfh_Data.getChoice_result_number("d7_choice_n1") == 1:
                ...
                dv "Хотя у меня такое ощущение, что в тот раз ты просто загляделся кое-куда."
            ...
            dv "Хорошо, тогда на площади вечером пересечёмся, а там посмотрим."
            me "Так и поступим."
        ...
        dv "Могу сразу утешить — беспокоиться не о чем. Никаких авантюр с печальными последствиями."
        me "Да неужели?"
        ...
        "Всё же я не до конца ей доверял, но[wp] Кто знает, может, действительно будет что-то интересное, за что я не отхвачу по самые помидоры."
    ...
    "С самодовольным видом Ульяна ткнула подругу в бок."
    ...
    if wnfh_Data.FlagGet("mt_angry") == True:
        ...
        usw "Так-так-так, и что это у вас за планы такие?"
        me "Не знаю."
...
label d9_zavtrak_w_un:
    ...
    if wnfh_Data.getChoice_result_number("d8_choice_n11") == 1:
        ...
        if wnfh_Data.getChoice_points_sum("un") >= 6:
            ...
            un "А что, если у меня не получится оформить газету как надо, и Женя будет ругаться?"
            ...
            me "Женя, может, чуть-чуть повозмущается, но орёт она только на раздолбаев по типу меня."

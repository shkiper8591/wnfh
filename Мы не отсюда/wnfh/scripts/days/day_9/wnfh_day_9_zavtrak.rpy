...
label d9_zavtrak_w_kat_mi:
    ...
    "Зайдя в столовую, я, как обычно, взял себе поесть и стал разглядывать куда бы усесться."

    if wnfh_Data.getChoice_points_sum("kat") >= 4:

        "Как заметил махающую мне Катю, что сидела рядом с Мику. Намёк был ясен."

    else:

        "Как заметил махающую мне Мику, что сидела рядом с Катей. Намёк был ясен."
    ...
    if wnfh_Data.FlagGet("mt_angry") == True:
        ...
        "Мои подруги тут же упали духом."
        ...
        "Далее, без особого энтузиазма, мы доели свои завтраки."
        "И, распрощавшись, я покинул столовую отправившись к складу."

        jump d9_warehouse

    else:
        
        me "Ну, дел особо сегодня нет, так что[wp] Почему бы и нет."
        ...
        if wnfh_Data.getChoice_result_number("d8_choice_n10") == 1:
            ...
            kat "Надеюсь посидим также хорошо, как вчера."
            me "Согласен."

        show mi normal pioneer at right with dspr

        mi "Ну что ж, в таком случае предлагаю прислушаться поговорке «когда я ем, я глух и нем!»."
        ...
        "Я же молча согласился с девушками и мы принялись уплетать наши завтраки."

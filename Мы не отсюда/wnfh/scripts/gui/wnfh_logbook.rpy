screen wnfh_logbook():
    #modal True #tag menu
    
    $ debug_frame = {
        "black":     frame_black      if persistent.wnfh_debug_color else frame_transparent,
        "red":       frame_red        if persistent.wnfh_debug_color else frame_transparent,
        "green":     frame_green      if persistent.wnfh_debug_color else frame_transparent,
        "blue":      frame_blue       if persistent.wnfh_debug_color else frame_transparent,
        "purple":    frame_purpl      if persistent.wnfh_debug_color else frame_transparent,
        "yellow":    frame_yellow     if persistent.wnfh_debug_color else frame_transparent,
        "turquoise": frame_turquoise  if persistent.wnfh_debug_color else frame_transparent,
    }
    
    default wnfh_button_states = [False for i in range(30)]

    python:
        mm_backgrounds = {
            "night":  wnfh_gui["main_menu"]["mm_bg_night"],
            "sunset": wnfh_gui["main_menu"]["mm_bg_sunset"],
            "day":    wnfh_gui["main_menu"]["mm_bg_day"],
        }

        if main_menu:
            wnfh_logbook_button = [
                ["back", "Назад", [ShowMenu('main_menu'), Hide('wnfh_logbook'), Hide("wnfh_logbook_main")]]
            ]
        else:
            wnfh_logbook_button = [
                ["back", "Назад", [ShowMenu('game_menu_selector'), Hide('wnfh_logbook'), Hide("wnfh_logbook_main")]]
            ]

        wnfh_bars = {
            "vtumb": [im.MatrixColor(wnfh_frames_elements["logbook_vbar_tumb"][0], im.matrix.tint(*converter_hex('wnfh_tint_color', wnfh_frames_elements["logbook_vbar_tumb"][4], renpy.store.wnfh_tymeofday)))],
            
            "bar_full": [im.Composite(
                (25, 25),
                (0, 0), im.MatrixColor(wnfh_frames_elements["logbook_vbar_full"][0], im.matrix.tint(*converter_hex('wnfh_tint_color', wnfh_frames_elements["logbook_vbar_full"][4], renpy.store.wnfh_tymeofday))),
                (0, 0), im.MatrixColor(wnfh_frames_elements["logbook_vbar_null"][0], im.matrix.tint(*converter_hex('wnfh_tint_color', wnfh_frames_elements["logbook_vbar_null"][4], renpy.store.wnfh_tymeofday))),
                )],
            "bar_null": [im.Composite(
                (25, 25),
                (0, 0), im.MatrixColor(wnfh_frames_elements["logbook_vbar_bg"][0], im.matrix.tint(*converter_hex('wnfh_tint_color', wnfh_frames_elements["logbook_vbar_bg"][4], renpy.store.wnfh_tymeofday))),
                (0, 0), im.MatrixColor(wnfh_frames_elements["logbook_vbar_null"][0], im.matrix.tint(*converter_hex('wnfh_tint_color', wnfh_frames_elements["logbook_vbar_null"][4], renpy.store.wnfh_tymeofday))),
                )],
        }

        wnfh_logbook_sections = [
            ["character_log", "Персонажи"       ],
            ["gallery",       "Галерея"         ],
            ["achievments",   "Достижения"      ],
        ]
    if main_menu:
        default current_hour = wnfh_get_usertime("hour") # ======================= Главное меню подстраивается под время суток компьютера

        default time_period = (
            "night"  if (current_hour >= 22 or current_hour < 8) else
            "sunset" if (current_hour < 12)                      else
            "day"    if (current_hour < 19)                      else
            "sunset"
        )

        frame:
            background mm_backgrounds[time_period] # ================== Фон в главном меню
            area(0.0, 0.0, 1.0, 1.0)

    
    on "show" action Show("wnfh_logbook_main")
    on "hide" action Hide("wnfh_logbook_main")

    
    frame at wjuh_bg:
        area(0.01, 0.11, 500, 0.98)
        xanchor 0.0 yanchor 0.0
        background debug_frame["red"]
        viewport id "logbook_char_list":
            draggable True
            mousewheel True
            scrollbars None
            vbox:
                pos (0.5, 0.5)
                xanchor 0.5 yanchor 0.5
                spacing 0
                for button in range(len(wnfh_logbook_sections)):
                    frame:
                        area (0.5, 0.0, wnfh_frames_elements["logbook_char_list_bg"][1] + 40, wnfh_frames_elements["logbook_char_list_bg"][2] + 20)
                        xanchor 0.5 yanchor 0.0
                        background debug_frame["blue"]
                        vbox: # ================================================ Фон таблички из трёх кусков
                            pos (0.5, 0.5)
                            xanchor 0.5 yanchor 0.5
                            spacing 0
                            for element in ["logbook_char_list_line", "logbook_char_list_bg", "logbook_char_list_line"]:
                                frame at wnfh_frames_elements[element][6]:
                                #frame:
                                    if persistent.wnfh_debug_color:
                                        background wnfh_frames_elements[element][5]
                                    else:
                                        background frame_transparent
                                    area (0.5, 0.0, wnfh_frames_elements[element][1], wnfh_frames_elements[element][2]) padding(0, 0) xanchor 0.5
                                    add Frame(wnfh_frames_elements[element][0], left=wnfh_frames_elements[element][3], top=0):
                                        matrixcolor TintMatrix(wnfh_tint_color[renpy.store.wnfh_tymeofday][wnfh_frames_elements[element][4]])
                        textbutton wnfh_logbook_sections[button][1] + " >": # ================================================ Текст кнопок
                            style "wnfh_buttons"
                            text_style "wnfh_ach_title_1_" + renpy.store.wnfh_tymeofday
                            text_text_align 1.0
                            #hovered ToggleDict(wnfh_button_states, character)
                            #unhovered ToggleDict(wnfh_button_states, character)
                            action None
                            at wnfh_mm_button_hover_atl()


screen wnfh_logbook_main():
    $ debug_frame = {
        "black":     frame_black      if persistent.wnfh_debug_color else frame_transparent,
        "red":       frame_red        if persistent.wnfh_debug_color else frame_transparent,
        "green":     frame_green      if persistent.wnfh_debug_color else frame_transparent,
        "blue":      frame_blue       if persistent.wnfh_debug_color else frame_transparent,
        "purple":    frame_purpl      if persistent.wnfh_debug_color else frame_transparent,
        "yellow":    frame_yellow     if persistent.wnfh_debug_color else frame_transparent,
        "turquoise": frame_turquoise  if persistent.wnfh_debug_color else frame_transparent,
    }
    
    default wnfh_button_states = [False for i in range(30)]

    on "show" action Function(update_usertime)
    frame at govno_ebanoe2:
        area(0.95, 0.11, 1250, 823)
        xanchor 1.0 yanchor 0.0
        background debug_frame["yellow"] 
        if persistent.wnfh_widget_clock:
            frame: # =============================================== Часики
                background debug_frame["black"]
                area(1.0, 0.0, wnfh_frames_elements["widget_clock_box_bg"][1] + 40, wnfh_frames_elements["widget_clock_box_bg"][2] + 20)
                xanchor 1.0 yanchor 0.0
                vbox: # ================================================ Фон таблички из трёх кусков
                    pos (0.5, 0.5)
                    xanchor 0.5 yanchor 0.5
                    spacing 0
                    for element in ["widget_clock_box_line", "widget_clock_box_bg", "widget_clock_box_line"]:
                        frame:
                            if persistent.wnfh_debug_color:
                                background wnfh_frames_elements[element][5]
                            else:
                                background frame_transparent
                            area(0.5, 0.0, wnfh_frames_elements[element][1], wnfh_frames_elements[element][2]) padding(0, 0) xanchor 0.5
                            add Frame(wnfh_frames_elements[element][0], left=wnfh_frames_elements[element][3], top=0):
                                matrixcolor TintMatrix(wnfh_tint_color[renpy.store.wnfh_tymeofday][wnfh_frames_elements[element][4]])
                hbox:
                    spacing 5
                    anchor (0.5, 0.5) pos (0.5, 0.5)
                    for element in ["hour", "separator", "mins"]:
                        frame:
                            background debug_frame["black"]
                            area(0.0, 0.5, 22, 40)
                            xanchor 0.0 yanchor 0.5
                            text str(wnfh_get_usertime(element)):
                                style "wnfh_text_" + renpy.store.wnfh_tymeofday
        
            timer 1:
                repeat True
                action Function(update_usertime)
    
        if persistent.wnfh_widget_music_player:
            #frame at atl_wnfh_widget_music: # =============================================== Музыка
            frame:
                background debug_frame["black"]
                area(0.0, 0.0, wnfh_frames_elements["widget_music_box_bg"][1] + 40, wnfh_frames_elements["widget_music_box_bg"][2] + 20)
                xanchor 0.0 yanchor 0.0
                vbox: # ================================================ Фон таблички из трёх кусков
                    pos (0.5, 0.5)
                    xanchor 0.5 yanchor 0.5
                    spacing 0
                    for element in ["widget_music_box_line", "widget_music_box_bg", "widget_music_box_line"]:
                        frame:
                            if persistent.wnfh_debug_color:
                                background wnfh_frames_elements[element][5]
                            else:
                                background frame_transparent
                            area(0.5, 0.0, wnfh_frames_elements[element][1], wnfh_frames_elements[element][2]) padding(0, 0) xanchor 0.5
                            add Frame(wnfh_frames_elements[element][0], left=wnfh_frames_elements[element][3], top=0):
                                matrixcolor TintMatrix(wnfh_tint_color[renpy.store.wnfh_tymeofday][wnfh_frames_elements[element][4]])
                text getMusicName():
                    xmaximum wnfh_frames_elements["widget_music_box_bg"][1]
                    style "wnfh_text_" + renpy.store.wnfh_tymeofday
        if persistent.wnfh_widget_lp:
            frame: # ================================================ Фрейм таблички
                area(1.0, 0.08, wnfh_frames_elements["widget_lp_box_bg"][1] + 40, wnfh_frames_elements["widget_lp_box_bg"][2] + 20)
                xanchor 1.0 yanchor 0.0
                background debug_frame["turquoise"]
                vbox: # ================================================ Фон таблички из трёх кусков
                    pos (0.5, 0.5)
                    xanchor 0.5 yanchor 0.5
                    spacing 0
                    for element in ["widget_lp_box_line", "widget_lp_box_bg", "widget_lp_box_line"]:
                        frame:
                            if persistent.wnfh_debug_color:
                                background wnfh_frames_elements[element][5]
                            else:
                                background frame_transparent
                            area(0.5, 0.0, wnfh_frames_elements[element][1], wnfh_frames_elements[element][2]) padding(0, 0) xanchor 0.5
                            add Frame(wnfh_frames_elements[element][0], left=wnfh_frames_elements[element][3], top=0):
                                matrixcolor TintMatrix(wnfh_tint_color[renpy.store.wnfh_tymeofday][wnfh_frames_elements[element][4]])
                grid 5 2: # ================================================ Ебальники с очками
                    spacing 5
                    anchor (0.5, 0.5) pos (0.5, 0.5)
                    
                    $ character_with_img = [character for character in wnfh_character_order]
                    for index, character in enumerate(character_with_img, start = 21 - len(character_with_img)):
                        frame:
                            background debug_frame["black"]
                            area(0.5, 0.5, 160, wnfh_frames_elements["widget_lp_box_bg"][2] / 2)
                            xanchor 0.5 yanchor 0.5
                            hbox: # ================================================ Ебальники с очками
                                spacing 0
                                anchor (0.5, 0.5) pos (0.5, 0.5)
                                frame: # ================================================ Ебальники
                                    if persistent.wnfh_debug_color:
                                        background wnfh_characters[character][1]
                                    else:
                                        background frame_transparent
                                    area(0.0, 0.5, 70, 90)
                                    xanchor 0.0 yanchor 0.5
                                    if renpy.store.wnfh_tymeofday == "day":
                                        add (wnfh_gui["avatars"][character]):
                                            xalign 0.5 yanchor 0.5 xpos 0.5 ypos 0.5
                                            zoom 0.1
                                    else:
                                        add (wnfh_gui["avatars"][character]):
                                            matrixcolor TintMatrix(wnfh_tint_color[renpy.store.wnfh_tymeofday][3])
                                            xalign 0.5 yanchor 0.5 xpos 0.5 ypos 0.5
                                            zoom 0.1
                                frame: # ================================================ Очки
                                    background debug_frame["blue"]
                                    area(1.0, 0.5, 60, 90)
                                    xanchor 1.0 yanchor 0.5
                                    text str(wnfh_Data.getChoice_points_sum(character)):
                                        style "wnfh_lp_counter"
                                        color wnfh_characters[character][1]
                        #frame: # ================================================ РАЗДЕЛИТЕЛЬ
                        #    area (0.5, 0.0, wnfh_frames_elements["logbook_char_list_bg_2"][1] + 40, wnfh_frames_elements["logbook_char_list_bg_2"][2] + 20)
                        #    xanchor 0.5 yanchor 0.0
                        #    background debug_frame["blue"]
                        #    vbox: # ================================================ Фон таблички из трёх кусков
                        #        pos (0.5, 0.5)
                        #        xanchor 0.5 yanchor 0.5
                        #        spacing 0
                        #        for element in ["logbook_char_list_line", "logbook_char_list_bg_2", "logbook_char_list_line"]:
                        #            frame at wnfh_frames_elements[element][6]:
                        #            #frame:
                        #                if persistent.wnfh_debug_color:
                        #                    background wnfh_frames_elements[element][5]
                        #                else:
                        #                    background frame_transparent
                        #                area (0.5, 0.0, wnfh_frames_elements[element][1], wnfh_frames_elements[element][2]) padding(0, 0) xanchor 0.5
                        #                add Frame(wnfh_frames_elements[element][0], left=wnfh_frames_elements[element][3], top=0):
                        #                    matrixcolor TintMatrix(wnfh_tint_color[renpy.store.wnfh_tymeofday][wnfh_frames_elements[element][4]])
                        #    text "Главные персонажи":
                        #        style "wnfh_text_" + renpy.store.wnfh_tymeofday
#
                        #$ character_order = ["kat", "un", "mi", "dv", "usw", "sl", "mt", "din", "sv", "mz"]
                        #$ character_with_img = [character for character in character_order]
                        #for element, character in enumerate(character_with_img, start = 21 - len(character_with_img)):
                        #    frame:
                        #        area (0.5, 0.0, wnfh_frames_elements["logbook_char_list_bg"][1] + 40, wnfh_frames_elements["logbook_char_list_bg"][2] + 20)
                        #        xanchor 0.5 yanchor 0.0
                        #        background debug_frame["blue"]
                        #        vbox: # ================================================ Фон таблички из трёх кусков
                        #            pos (0.5, 0.5)
                        #            xanchor 0.5 yanchor 0.5
                        #            spacing 0
                        #            for element in ["logbook_char_list_line", "logbook_char_list_bg", "logbook_char_list_line"]:
                        #                frame at wnfh_frames_elements[element][6]:
                        #                #frame:
                        #                    if persistent.wnfh_debug_color:
                        #                        background wnfh_frames_elements[element][5]
                        #                    else:
                        #                        background frame_transparent
                        #                    area (0.5, 0.0, wnfh_frames_elements[element][1], wnfh_frames_elements[element][2]) padding(0, 0) xanchor 0.5
                        #                    add Frame(wnfh_frames_elements[element][0], left=wnfh_frames_elements[element][3], top=0):
                        #                        matrixcolor TintMatrix(wnfh_tint_color[renpy.store.wnfh_tymeofday][wnfh_frames_elements[element][4]])
#
                        #        
                        #        frame: # ================================================ Тонировка при наведении
                        #            if wnfh_button_states[index]:
                        #                add Frame(wnfh_frames_elements["logbook_char_list_gradient"][0], left=wnfh_frames_elements["logbook_char_list_gradient"][3], right=400, top=0):
                        #                    xalign 0.5 yalign 0.5 alpha 0.6
                        #                    matrixcolor TintMatrix(wnfh_characters[character][1])
                        #                    at wnfh_gradient
                        #                add Frame(wnfh_frames_elements["logbook_char_list_gradient"][0], left=wnfh_frames_elements["logbook_char_list_gradient"][3], right=400, top=0):
                        #                    xalign 0.5 yalign 0.5 alpha 0.1
                        #                    at wnfh_gradient
                        #            else:
                        #                add Frame(wnfh_frames_elements["logbook_char_list_gradient"][0], left=wnfh_frames_elements["logbook_char_list_gradient"][3], right=400, top=0):
                        #                    xalign 0.5 yalign 0.5 alpha 0.6
                        #                    matrixcolor TintMatrix(wnfh_characters[character][1])
                        #                    at wnfh_gradient
                        #                add Frame(wnfh_frames_elements["logbook_char_list_gradient"][0], left=wnfh_frames_elements["logbook_char_list_gradient"][3], right=400, top=0):
                        #                    xalign 0.5 yalign 0.5 alpha 0.1
                        #                    at wnfh_gradient
                        #            area(0.5, 0.5, wnfh_frames_elements["logbook_char_list_bg"][1], wnfh_frames_elements["logbook_char_list_bg"][2]) padding(0, 0) xanchor 0.5 yanchor 0.5
                        #            background debug_frame["purple"]
                        #            textbutton wnfh_characters[character][0]: # ================================================ Текст кнопок
                        #                style "wnfh_buttons"
                        #                text_style "wnfh_ach_title_1_" + renpy.store.wnfh_tymeofday
                        #                text_text_align 1.0
                        #                hovered ToggleDict(wnfh_button_states, character)
                        #                unhovered ToggleDict(wnfh_button_states, character)
                        #                #action button[2]
                        #                at wnfh_mm_button_hover_atl()
                        #        viewport:
                        #            xmaximum 200
                        #            ymaximum 80
                        #        
                        #            pos (0.01, 0.5)
                        #            xanchor 0.0 yanchor 0.5
                        #        
                        #            add debug_frame["green"]
                        #        
                        #            if renpy.store.wnfh_tymeofday == "day":
                        #                add Transform(
                        #                    wnfh_gui["avatars"][character],
                        #                    xpos = 0.5, ypos = 0.8,
                        #                    xanchor = 0.5, yanchor = 0.5,
                        #                    zoom = 0.25
                        #                )
                        #            else:
                        #                add Transform(
                        #                    wnfh_gui["avatars"][character],
                        #                    matrixcolor = TintMatrix(wnfh_tint_color[renpy.store.wnfh_tymeofday][3]),
                        #                    xpos = 0.5, ypos = 0.8,
                        #                    xanchor = 0.5, yanchor = 0.5,
                        #                    zoom = 0.25
                        #                )
                #frame:
                #    background debug_frame["green"]
                #    area(1.0, 0.5, 50, 1.0)
                #    xanchor 0.0 yanchor 0.5
                #    vbar value YScrollValue("logbook_char_list"):
                #        top_bar Frame(wnfh_bars["bar_null"][0], wnfh_frames_elements["logbook_vbar_null"][1], wnfh_frames_elements["logbook_vbar_null"][1])
                #        bottom_bar Frame(wnfh_bars["bar_null"][0], wnfh_frames_elements["logbook_vbar_null"][1], wnfh_frames_elements["logbook_vbar_null"][1])
                #        thumb wnfh_bars["vtumb"][0]
                #        hover_thumb wnfh_bars["vtumb"][0]
                #        xmaximum 33 ymaximum 1.0
                #        pos (0.5, 0.5)
                #        anchor (0.5, 0.5)
#
            #frame:
            #    area (0.45, 0.0, 600, 600)
            #    xanchor 0.5 yanchor 0.0
            #    background debug_frame["green"]
            #    if renpy.store.wnfh_tymeofday == "day":
            #        add wnfh_gui["avatars"]["mi"]:
            #            xanchor 0.5 yanchor 0.5 xpos 0.5 ypos 0.5
            #            zoom 0.7
            #    else:
            #        add wnfh_gui["avatars"]["mi"]:
            #            matrixcolor TintMatrix(wnfh_tint_color[renpy.store.wnfh_tymeofday][3])
            #            xanchor 0.5 yanchor 0.5 xpos 0.5 ypos 0.5
            #            zoom 0.7
            #frame:
            #    area(0.45, 1.0, 600, 200)
            #    xanchor 0.5 yanchor 1.0
            #    background debug_frame["blue"]
            #    text "какой-то текст":
            #        style "wnfh_ach_title_2_" + renpy.store.wnfh_tymeofday
            #frame:
            #    area(0.95, 0.0, 600, 600)
            #    xanchor 1.0 yanchor 0.0
            #    background debug_frame["green"]
            #    text "Галерея":
            #        style "wnfh_text_" + renpy.store.wnfh_tymeofday
            #frame:
            #    area(0.95, 1.0, 600, 200)
            #    xanchor 1.0 yanchor 1.0
            #    background debug_frame["purple"]
            #    text "Достижения":
            #        style "wnfh_text_" + renpy.store.wnfh_tymeofday